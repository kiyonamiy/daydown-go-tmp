...
gen.add-copyright: ## 添加版权头信息.
	@addlicense -v -f $(ROOT_DIR)/scripts/boilerplate.txt -ignore third_party/** -ignore vendor/** -ignore $(OUTPUT_DIR) $(ROOT_DIR)
...
gen.ca: ## 生成 CA 文件.
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/ca.key -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=Zhejiang/L=Hangzhou/O=devops/OU=it/CN=127.0.0.1/emailAddress=yuqingbo0122@gmail.com" # 2. 生成请求文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=Zhejiang/L=Hangzhou/O=serverdevops/OU=serverit/CN=127.0.0.1/emailAddress=yuqingbo0122@gmail.com" # 6. 生成服务端向 CA 申请签名的 CSR
